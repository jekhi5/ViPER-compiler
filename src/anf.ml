...
type 'a anf_bind =
  ...
  | BLetRec of (string * 'a cexpr) list
      (** Introduce a group of mutually-recursive bindings (each rhs must be a lambda). *)

(** [anf p] converts a tag-annotated source [program] into an A-Normal Form program with sourcespan
