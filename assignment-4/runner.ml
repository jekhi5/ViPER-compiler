...
let test_run ?(args = []) ?(std_input = "") program_str outfile expected ?(cmp = ( = )) _test_ctxt =
  let full_outfile = "output/" ^ outfile in
  let result =
    try
      let program = parse_string outfile program_str in
      run program full_outfile run_no_vg args std_input
...
let test_run_valgrind
  ...
  let result =
    try
      let program = parse_string outfile program_str in
      run program full_outfile run_vg args std_input
    with err -> Error (Printexc.to_string err)
  in
  assert_equal (Ok (expected ^ "\n")) result ~cmp ~printer:result_printer
;;

let test_err ?(args = []) ?(std_input = "") program_str outfile errmsg ?(vg = false) _test_ctxt =
  let full_outfile = "output/" ^ outfile in
  let result =
    try
      let program = parse_string outfile program_str in
      let runner =
        if vg then
          ...
          run_no_vg
      in
      run program full_outfile runner args std_input
    with err -> Error (Printexc.to_string err)
  in
...
let test_run_input filename ?(args = []) expected test_ctxt =
  test_run ~args ~std_input:"" (string_of_file ("input/" ^ filename)) filename expected test_ctxt
;;

let test_err_input filename ?(args = []) expected test_ctxt =
  test_err ~args ~std_input:"" (string_of_file ("input/" ^ filename)) filename expected test_ctxt
;;
...
let parse_args (argsfile : string) : string list =
...
let test_does_run filename test_ctxt =
  ...
  let progfile = sprintf "input/do_pass/%s.cobra" filename in
  ...
  let valgrind = sprintf "input/do_pass/%s.valgrind" filename in
  let prog = string_of_file progfile in
  let args = parse_args argsfile in
  let output =
  ...
  in
  let runner =
    if Sys.file_exists valgrind then
      test_run_valgrind
    else
      test_run
  in
  runner ~args ~std_input:input prog ("do_pass/" ^ filename) output test_ctxt
...
let test_does_err filename test_ctxt =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "input/do_err/%s.cobra" filename in
  ...
  let input =
    if Sys.file_exists infile then
      ...
      ""
  in
  test_err ~args ~std_input:input prog ("do_err/" ^ filename) err ~vg:(Sys.file_exists valgrind)
    test_ctxt
;;

let test_doesnt_run filename _test_ctxt =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "input/dont_pass/%s.cobra" filename in
  ...
  let result =
    try
      ...
      run program full_outfile runner args input
    with err -> Error (Printexc.to_string err)
  in
  match result with
  | Ok unexpected ->
...
let test_doesnt_err filename _test_ctxt =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "input/dont_err/%s.cobra" filename in
...
let input_file_test_suite () =
  ...
  "input-file-suite"
  >::: [ "do_pass"
         >::: List.map (fun f -> f >:: test_does_run f) (safe_readdir "input/do_pass" ".cobra");
         "do_err"
         >::: List.map (fun f -> f >:: test_does_err f) (safe_readdir "input/do_err" ".cobra");
         "dont_pass"
         >::: List.map (fun f -> f >:: test_doesnt_run f) (safe_readdir "input/dont_pass" ".cobra");
         "dont_err"
         >::: List.map (fun f -> f >:: test_doesnt_err f) (safe_readdir "input/dont_err" ".cobra")
       ]
;;
