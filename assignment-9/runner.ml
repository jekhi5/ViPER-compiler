...
let compile_string_to_string ?(no_builtins = false) (name : string) (input : string) :
    string pipeline =
  Ok (input, [])
  |> add_phase source (fun x -> x)
  |> add_err_phase parsed (fun input -> try Ok (parse_string name input) with err -> Error [err])
  |> compile_to_string ~no_builtins
;;

let compile_file_to_string ?(no_builtins = false) (name : string) (input_file : string) :
    string pipeline =
  compile_string_to_string ~no_builtins name (string_of_file input_file)
;;
...
let run p out runner no_builtins args std_input =
  let maybe_asm_string = compile_to_string ~no_builtins (Ok (p, [])) in
...
let read_options filename : compile_opts =
  ...
  in
  {valgrind= List.mem "valgrind" opts; no_builtins= List.mem "no_builtins" opts; heap_size}
;;
...
let test_does_run filename test_ctxt =
  ...
  let runner =
    if opts.valgrind then
      ...
      test_run
  in
  runner ~no_builtins:opts.no_builtins ~args ~std_input:input prog ("do_pass/" ^ filename) output
    test_ctxt ~cmp:(fun check result ->
      match (check, result) with
...
let test_does_err filename test_ctxt =
  ...
  let input =
    if Sys.file_exists infile then
      ...
      ""
  in
  test_err ~no_builtins:opts.no_builtins ~args ~std_input:input prog ("do_err/" ^ filename) err
    ~vg:opts.valgrind test_ctxt
;;
...
let input_file_test_suite () =
  ...
  "input-file-suite"
  >::: [ "do_pass"
         >::: List.map (fun f -> f >:: test_does_run f) (safe_readdir "input/do_pass" ".garter");
         "do_err"
         >::: List.map (fun f -> f >:: test_does_err f) (safe_readdir "input/do_err" ".garter");
         "dont_pass"
         >::: List.map (fun f -> f >:: test_doesnt_run f) (safe_readdir "input/dont_pass" ".garter");
         "dont_err"
         >::: List.map (fun f -> f >:: test_doesnt_err f) (safe_readdir "input/dont_err" ".garter")
       ]
;;
