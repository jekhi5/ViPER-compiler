...
let free_vars (e : 'a aexpr) : StringSet.t =
  let rec immexpr_free_vars (e : 'a immexpr) (bound_vars : StringSet.t) : StringSet.t =
    match e with
    | ImmNum _ -> StringSet.empty
    | ImmBool _ -> StringSet.empty
    | ImmId (str, _) ->
        if StringSet.mem str bound_vars then
          StringSet.empty
        else
          StringSet.of_list [str]
    | ImmNil _ -> StringSet.empty
  and cexpr_free_vars (e : 'a cexpr) (bound_vars : StringSet.t) : StringSet.t =
    match e with
    | CIf (cond, thn, els, _) ->
        let cond_set = immexpr_free_vars cond bound_vars in
        let thn_set = aexpr_free_vars thn bound_vars in
        let els_set = aexpr_free_vars els bound_vars in
        StringSet.union (StringSet.union cond_set thn_set) els_set
    | CPrim1 (_, expr, _) -> immexpr_free_vars expr bound_vars
    | CPrim2 (_, expr1, expr2, _) ->
        let expr1_set = immexpr_free_vars expr1 bound_vars in
        let expr2_set = immexpr_free_vars expr2 bound_vars in
        StringSet.union expr1_set expr2_set
    | CApp (func, args, _, _) ->
        let func_set = immexpr_free_vars func bound_vars in
        let args_set =
          List.fold_right
            (fun arg old_set -> StringSet.union (immexpr_free_vars arg bound_vars) old_set)
            args StringSet.empty
        in
        StringSet.union func_set args_set
    | CImmExpr immexpr -> immexpr_free_vars immexpr bound_vars
    | CTuple (items, _) ->
        List.fold_right
          (fun arg old_set -> StringSet.union (immexpr_free_vars arg bound_vars) old_set)
          items StringSet.empty
    | CGetItem (tuple, index, _) ->
        let tuple_set = immexpr_free_vars tuple bound_vars in
        let index_set = immexpr_free_vars index bound_vars in
        StringSet.union tuple_set index_set
    | CSetItem (tuple, index, new_val, _) ->
        let tuple_set = immexpr_free_vars tuple bound_vars in
        let index_set = immexpr_free_vars index bound_vars in
        let new_val_set = immexpr_free_vars new_val bound_vars in
        StringSet.union (StringSet.union tuple_set index_set) new_val_set
    | CLambda (args, body, _) ->
        let new_bound_vars = StringSet.union (StringSet.of_list args) bound_vars in
        aexpr_free_vars body new_bound_vars
  and aexpr_free_vars (e : 'a aexpr) (bound_vars : StringSet.t) : StringSet.t =
    match e with
    | ASeq (expr1, expr2, _) ->
        let expr1_set = cexpr_free_vars expr1 bound_vars in
        let expr2_set = aexpr_free_vars expr2 bound_vars in
        StringSet.union expr1_set expr2_set
    | ALet (id, assign_val, body, _) ->
        let assign_val_set = cexpr_free_vars assign_val bound_vars in
        let new_seen_vars = StringSet.add id bound_vars in
        let body_set = aexpr_free_vars body new_seen_vars in
        StringSet.union assign_val_set body_set
    | ACExpr cexpr -> cexpr_free_vars cexpr bound_vars
    | ALetRec (bindings, body, _) ->
        let new_seen_vars, free_var_bindings =
          List.fold_right
            (fun binding (old_bound_vars, old_free_vars) ->
              let id, assign_value = binding in
              let seen_vars = StringSet.add id old_bound_vars in
              let assign_value_set = cexpr_free_vars assign_value seen_vars in
              (seen_vars, StringSet.union assign_value_set old_free_vars) )
            bindings (bound_vars, StringSet.empty)
        in
        let body_set = aexpr_free_vars body new_seen_vars in
        StringSet.union free_var_bindings body_set
  in
  aexpr_free_vars e StringSet.empty
;;
