  ...
  | DFun (name, args, body, a) ->
      sprintf "(def %s(%s):\n  %s)%s" name
        (ExtString.String.join ", "
           (List.map (fun (arg, a) -> sprintf "%s%s" arg (print_a a)) args) )
...
let format_expr (e : 'a expr) (print_a : 'a -> string) : string =
  ...
  in
  let print_comma_sep fmt = pp_print_string fmt ","; pp_print_space fmt () in
  let open_label fmt label a =
    pp_open_hvbox fmt indent;
    pp_print_string fmt label;
    pp_print_string fmt (maybe_a a);
    pp_print_string fmt "(";
    pp_print_cut fmt ()
  in
  let open_paren fmt = pp_open_box fmt 2; pp_print_string fmt "("; pp_print_cut fmt () in
  let close_paren fmt =
  ...
  let rec help e fmt =
    ...
    | ENumber (n, a) ->
        ...
        close_paren fmt
    | EBool (b, a) -> open_label fmt "EBool" a; pp_print_bool fmt b; close_paren fmt
    | EId (x, a) ->
    ...
    | EApp (funname, args, a) ->
        ...
        | e1 :: rest ->
            help e1 fmt;
            List.iter (fun e -> print_comma_sep fmt; help e fmt) rest );
        close_paren fmt
    | ELet (binds, body, a) ->
        ...
        close_paren fmt
  in
  help e str_formatter; flush_str_formatter ()
;;

let ast_of_pos_expr (e : (Lexing.position * Lexing.position) expr) : string =
  format_expr e string_of_sourcespan
;;

let ast_of_expr (e : 'a expr) : string = format_expr e (fun _ -> "")
