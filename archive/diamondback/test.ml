...
(* Since we only use if for a couple tests, the printer is useless to avoid opening Extlib for `dump`. *)
let t_any name value expected = name >:: fun _ -> assert_equal expected value ~printer:(fun _ -> "")
...
let anf_tests =
  [ tanf "prim2_anf"
    ...
    tanf "decl_first_degree"
      (Program ([DFun ("foo", [("x", 0)], EId ("x", 0), 0)], EApp ("foo", [ENumber (1L, 0)], 0), 0))
      (AProgram
