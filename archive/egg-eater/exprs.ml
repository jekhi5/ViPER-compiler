...
and 'a bind =
  | BBlank of 'a
  (* You can safely ignore the boolean in here for Egg-Eater; it will become
     more relevant next assignment but is included now to prevent changing
     data definitions.
     If you are curious, you can examine parser.mly and attempt to give a semantics:
     it gives users influence over whether or not an identifier can be shadowed *)
  | BName of string * bool * 'a
  | BTuple of 'a bind list * 'a

and 'a binding = 'a bind * 'a expr * 'a

(* as mentioned in compile.ml, this data definition allows us to distinguish
   between internal Snake functions, external Native functions, primitive
   Prim functions, and functions which we do not know information about (yet).

   These categories of functions may be handled and called in different ways,
   motivating the presence of this data type. *)
and call_type =
  | Native
  | Snake
  | Prim
...
let rec untagP (p : 'a program) : unit program =
  match p with
  | Program (decls, body, _) -> Program (List.map untagD decls, untagE body, ())
...
and untagE e =
  ...
  | EApp (name, args, native, _) -> EApp (name, List.map untagE args, native, ())
