...
let read_options filename : compile_opts =
  ...
  in
  {valgrind= List.mem "valgrind" opts; heap_size}
;;

let parse_args (argsfile : string) (_opts : compile_opts) : string list =
...
let test_run ?(args = []) ?(std_input = "") program_str outfile expected ?(cmp = ( = )) _ =
...
let test_run_valgrind ?(args = []) ?(std_input = "") program_str outfile expected ?(cmp = ( = )) _ =
  let full_outfile = "output/" ^ outfile in
  let result =
    try
      let program = parse_string outfile program_str in
...
let test_err ?(args = []) ?(std_input = "") program_str outfile errmsg ?(vg = false) _ =
  let full_outfile = "output/" ^ outfile in
  let runner =
    if vg then
      run_vg
...
let test_does_run filename test_ctxt =
  ...
  let progfile = sprintf "input/do_pass/%s.egg" filename in
...
let test_does_err filename test_ctxt =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "input/do_err/%s.egg" filename in
  ...
  let input =
    if Sys.file_exists infile then
      ...
      ""
  in
  test_err ~args ~std_input:input prog ("do_err/" ^ filename) err ~vg:opts.valgrind test_ctxt
;;

let test_doesnt_run filename _ =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "input/dont_pass/%s.egg" filename in
...
let test_doesnt_err filename _ =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "input/dont_err/%s.egg" filename in
...
let input_file_test_suite () =
  ...
  "input-file-suite"
  >::: [ "do_pass"
         >::: List.map (fun f -> f >:: test_does_run f) (safe_readdir "input/do_pass" ".egg");
         "do_err"
         >::: List.map (fun f -> f >:: test_does_err f) (safe_readdir "input/do_err" ".egg");
         "dont_pass"
         >::: List.map (fun f -> f >:: test_doesnt_run f) (safe_readdir "input/dont_pass" ".egg");
         "dont_err"
         >::: List.map (fun f -> f >:: test_doesnt_err f) (safe_readdir "input/dont_err" ".egg") ]
;;
