...
type compile_opts = {valgrind: bool}
...
let test_does_run filename test_ctxt =
  ...
  let progfile = sprintf "input/do_pass/%s.diamond" filename in
...
let test_does_err filename test_ctxt =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "input/do_err/%s.diamond" filename in
...
let test_doesnt_run filename _ =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "input/dont_pass/%s.diamond" filename in
...
let test_doesnt_err filename _ =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "input/dont_err/%s.diamond" filename in
...
let input_file_test_suite () =
  ...
  "input-file-suite"
  >::: [ "do_pass"
         >::: List.map (fun f -> f >:: test_does_run f) (safe_readdir "input/do_pass" ".diamond");
         "do_err"
         >::: List.map (fun f -> f >:: test_does_err f) (safe_readdir "input/do_err" ".diamond");
         "dont_pass"
         >::: List.map
                (fun f -> f >:: test_doesnt_run f)
                (safe_readdir "input/dont_pass" ".diamond");
         "dont_err"
         >::: List.map (fun f -> f >:: test_doesnt_err f) (safe_readdir "input/dont_err" ".diamond")
       ]
;;
